...
|--------|--------|----|------|---------|
| 初始化耗时（秒） | 0.044835 | 0.024628 | 0.031008 | 0.0436163 |
| 插入50万条总耗时（秒） | 17.790447 | 17.919936 | 8.337171 | 9.32522 |
| 单条主键查询（秒/次） | 0.000031 | 0.000035 | 0.000059 | 0.0000438995 |
| 条件查询（秒/次） | 0.000073 | 0.000051 | 0.000043 | 0.0000398984 |
| 范围查询（秒） | 0.033728 | 0.035364 | 0.019348 | 0.0433786 |
| 更新性能（秒/次） | 0.000104 | 0.000062 | 0.000031 | 0.000132715 |
| 删除性能（秒/次） | 0.000029 | 0.000015 | 0.000021 | 0.0000893919 |
| 并发总耗时（秒） | 4.429592 | 3.490551 | 5.503257 | 5.48928 |


#### （2）关键现象与原因分析
1. **初始化性能**  
   Go初始化耗时最短（0.0246秒），得益于其轻量级运行时和`lib/pq`驱动的高效连接管理；C++和Python因编译期检查（C++）和解释器启动开销（Python）略长。

2. **批量插入性能**  
   Java表现最优（8.34秒），远超其他语言，原因在于JDBC连接池（HikariCP）的连接复用机制和`PreparedStatement`的批量处理优化，减少了数据库交互次数；Python和Go耗时接近（约17.8秒），受限于驱动对事务提交的处理效率。

3. **查询性能**  
   - 单条主键查询：Python和Go最快（约0.00003秒/次），因驱动对索引查询的直接映射；Java因JVM内存管理开销略慢。  
   - 条件查询：C++和Java更优（0.00004秒/次级），得益于底层驱动对索引扫描的高效实现；Python因动态类型解析开销略高。  
   - 范围查询：Java最快（0.019秒），可能因JIT编译对结果集处理循环的优化；C++因内存分配开销略慢。

4. **更新与删除性能**  
   - 更新：Java最优（0.000031秒/次），Go次之，C++因事务处理逻辑较重（需手动管理连接状态）最慢。  
   - 删除：Go表现最佳（0.000015秒/次），因`lib/pq`驱动对删除操作的轻量级封装；C++因指针操作和资源释放开销较高。
